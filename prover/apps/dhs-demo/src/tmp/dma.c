...
static inline void
_dma_fill(seL4_Word pstart, seL4_Word pend, int cached, dma_man_t *dma){
    ...
    while(pstart < pend){
        if(*caps == seL4_CapNull){
            ...
            err = vka_alloc_frame(&(dma->vka), seL4_PageBits, &frame_obj);
            *caps = frame_obj.cptr;
            ...
            assert(!err);
            /* Map in the frame */
            err = map_page(&(dma->vka), *caps, seL4_CapInitThreadPD,
                            VIRT(pstart),
                            seL4_AllRights,
                            vm_attr);
...
sos_dma_malloc(void* cookie, size_t size, int align, int cached, ps_mem_flags_t flags) {
    ...
    if(_dma_pnext < _dma_pend){
        ...
        alloc_cached = cached;
        /* Fill the dma memory */
        _dma_fill(_dma_pnext, _dma_pnext + size, cached, dma);
        /* set return values */
