...
/*static uint32_t compute_checksum(paddr_t dest_paddr, uint32_t image_size) {

    int checksum = 0;
    int i = 0;
    for(i=0; i<(int)image_size; i++) {
        checksum += (int)(*(char*)(dest_paddr+i));
    }
    printf("Checksum p = %d, start %x, end %x, size %x\n", checksum, dest_paddr, dest_paddr+image_size, image_size);
                        ...
                        paddr_t dest_paddr, struct image_info *info)
{
    ...
    //compute_checksum(dest_paddr, image_size);
